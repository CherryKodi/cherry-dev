...
class source:
    ...
    def movie(self, imdb, title, localtitle, aliases, year):
        return [self.clean_serach(title), self.clean_serach(localtitle), year]
    ...
    def sources(self, url, hostDict, hostprDict):
        try:
            ...
            for url_single in titles:
                ...
                for el in result :
                    found_title = str(client.parseDOM(el, 'div', attrs={'class':'title'})[0]).lower()
                    if ("zwiastun" or "trailer") in str(found_title).lower():
                        continue
                    if len(words) >= 4:
                        regtest = re.findall("\d{3}[^1080p]", found_title)
                        if len(regtest) > 0:
                            if not self.contains_all_wors(found_title, words) and year not in found_title:
                                continue
                        else:
