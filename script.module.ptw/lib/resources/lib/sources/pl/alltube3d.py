...
class source:
    ...
    def search(self, title, localtitle, year, search_type):
        try:
            ...
            r = client.request(urlparse.urljoin(self.base_link, self.search_link), post={'search': cleantitle.query(title + " 3d")},proxy = self.proxy)
            r = self.get_rows(r, search_type)
            localtitle = cleantitle.normalize(cleantitle.getsearch(localtitle + " 3d"))
            ...
            for row in r:
                ...
                for name in names_found:
                    ...
                    if (self.contains_all_wors(name, words) or self.contains_all_wors(name, words2)) and (not found_year or found_year == year):
                            return url
                    else:
                        continue
                continue
        ...
        try:
            ...
            for i in links:
                try:
                    ...
                    sources.append({'source': i[1].encode('utf-8'), 'quality': 'SD', 'language': language, 'url': url1, 'info': info +" 3D", 'direct': False, 'debridonly': False})
                except:
                    pass
    ...
    def resolve(self, url):
        try:
            myurl = url.split('?')
            mycookie = client.request(myurl[0], output='cookie', error=True, proxy= self.proxy)
            ...
            data = client.request(urlparse.urljoin(self.base_link, '/jsverify.php?op=tag'), cookie=mycookie, proxy = self.proxy)
